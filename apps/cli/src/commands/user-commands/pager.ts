...
const args = {
  set: {
    demandOption: false,
    type: 'string',
    describe: 'Set default pager for Pancake. e.g. --set "less -FRX".',
  },
  disable: {
    demandOption: false,
    default: false,
    type: 'boolean',
    describe: 'Disable pager for Pancake',
  },
  unset: {
    demandOption: false,
    default: false,
    type: 'boolean',
    describe: 'Unset default pager for Pancake and default to git pager.',
  },
} as const;

type argsT = yargs.Arguments<yargs.InferredOptionTypes<typeof args>>;
export const command = 'pager';
export const description = 'The pager opened by Pancake.';
  ...
  return graphiteWithoutRepo(argv, canonical, async (context) => {
    if (argv.disable) {
      ...
      !currentPager
        ...
        : context.splog.info(
            `Pager is not set. Pancake will use your git pager (currently ${chalk.cyan(
              currentPager
            )})`
          );
    }
