...
export const description =
  'Authenticate with the GitHub CLI to create and manage PRs in GitHub from Pancake.';
...
export const handler = async (argv: argsT): Promise<void> => {
  return graphiteWithoutRepo(argv, canonical, async (context) => {
    ...
    context.splog.message(
      '❌ Pancake is not authenticated with GitHub. Please authenticate.'
    );
    ...
    try {
      ...
      context.splog.message(
        '✅ Successfully authenticated Pancake with GitHub.'
      );
    } catch {
      context.splog.message(
        '❌ Failed to authenticate Pancake with GitHub. Please try again.'
      );
    }
  });
};
